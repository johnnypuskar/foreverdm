...
class StatblockWrapper:
    ...
    def add_ability(self, ability):
        return self._ability_handler.add_ability(ability)
    
    def remove_ability(self, ability_name):
        return self._ability_handler.remove_ability(ability_name)
    
    def use_ability(self, ability_name, *args):
        return self._ability_handler.use_ability(ability_name, *args)
    
    # Ability Roll Handler
    def ability_check(self, dc, ability_name, target = None):
        return self._ability_roll_handler.ability_check(dc, ability_name, target)
    
    def skill_check(self, dc, skill_name, target = None, ability_name: str = None):
        return self._ability_roll_handler.skill_check(dc, skill_name, target, ability_name)
    
    def saving_throw(self, dc, ability_name, trigger = None):
        return self._ability_roll_handler.saving_throw(dc, ability_name, trigger)
    ...
    def melee_attack_roll(self, target, damage_string):
        return self._attack_roll_handler.melee_attack_roll(target, damage_string)
    
    def ranged_attack_roll(self, target, damage_string):
        return self._attack_roll_handler.ranged_attack_roll(target, damage_string)
    
    def ability_attack_roll(self, target, attack_stat, damage_string):
        return self._attack_roll_handler.ability_attack_roll(target, attack_stat, damage_string)
    
    # Effect Handler
    def add_effect(self, effect, duration):
        return self._effect_handler.add_effect(effect, duration)
    
    def remove_effect(self, effect_name):
        return self._effect_handler.remove_effect(effect_name)

    def add_condition(self, condition, duration):
        return self._effect_handler.add_condition(condition, duration)
    
    def remove_condition(self, condition_name):
        return self._effect_handler.remove_condition(condition_name)
    ...
    def restore_hp(self, amount):
        return self._hit_point_handler.restore_hp(amount)
    
    def take_damage(self, damage_string):
        return self._hit_point_handler.take_damage(damage_string)
